...
def train(model, annotation_path, input_shape, anchors, num_classes, log_dir='logs/'):
    ...
    num_val = int(len(lines) * cfg.ROOT.VAL_SPLIT)
    num_train = len(lines) - num_val
    print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, cfg.ROOT.BATCH_SIZE))

    model.fit_generator(data_generator_wrap(lines[:num_train], cfg.ROOT.BATCH_SIZE, input_shape, anchors, num_classes),
                        steps_per_epoch=max(1, num_train // cfg.ROOT.BATCH_SIZE),
                        validation_data=data_generator_wrap(lines[num_train:], cfg.ROOT.BATCH_SIZE, input_shape, anchors,
                                                            num_classes),
                        validation_steps=max(1, num_val // cfg.ROOT.BATCH_SIZE),
                        epochs=cfg.ROOT.EPOCHS,
                        initial_epoch=cfg.ROOT.INITIAL_EPOCHS,
